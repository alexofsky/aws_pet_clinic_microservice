            ...
            Parameters=[
                {
                    ...
                    #'ParameterValue': '3',
                    # Changing the max auto scaling size to 4 instead of 3 to logically balance micro services (4) with multiple instances
                    # across 4 EC2 instances.
                    'ParameterValue': '4',
                ...
                },
                {
                    ...
                    # Changing default to t2.medium instances.
                    # make sure memory allocation within container definitions for EC2 instances makes sense for 
                    # EC2 max resources as defined by Amazon.   The setting around 710 in this script.  For t2.medium set to 512MB.
                    #You could try out with t2.micro instances, but I didn't have great success with the target groups
                    #spinning up and remaining healthy.
...
def docker_login_config():
    ...
    with open(filename, 'r+') as f:
        data = json.load(f)
        #MJR needed tp add str conversion calls
        data['auths'] = {
            str(hostname, 'utf-8'): {
...
def setup(project_name='spring-petclinic-rest',
    ...
    logger.info('Logger create status = [ ' + stack_create_status['Stacks'][0]['StackStatus'] + ' ]'
    ...
    for resource in resources['StackResources']:
        logger.info('Logger resource from stack resource = [ ' + resource['LogicalResourceId'] + ' ]')
    ...
    elb_dns = create_elb_response['LoadBalancers'][0]['DNSName']
    ...
    logger.info("ELB available and active")
    ...
    default_target_group_arn = create_default_target_group_response['TargetGroups'][0]['TargetGroupArn']
    ...
    for service in service_list:
        ...
        if service == 'spring-petclinic-rest-system':
            ...
            logger.info("Target group response  = [ " + json.dumps(create_target_group_response) + " ]")
            ...
            logger.info("ELB Target Group created: [ " + target_group_arn + " ]")
        ...
        containerDefinitions = [
            {
                ...
                'environment': [
                    ...
                    },
                    {
                        ...
                        'value': str(my_sql_options['dns_name'])
                    },
                    {
                        'name': 'SPRING_DATASOURCE_USERNAME',
                        'value': str(my_sql_options['username'])
                    },
                    {
                        'name': 'SPRING_DATASOURCE_PASSWORD',
                        'value': str(my_sql_options['password'])
                    }
                ],
                'dockerLabels': {
                    'string': 'string'
